...
namespace ProvidenceMod.Projectiles.Melee
{
	public class StratusSpear : ModProjectile
	{
		...
		{
			Main.spriteBatch.Draw(Request<Texture2D>("ProvidenceMod/Projectiles/Melee/StratusSpear").Value, Projectile.Center - Main.screenPosition, new Rectangle(0, 0, 146, 42), Color.White, Projectile.rotation, new Vector2(Projectile.width, Projectile.height) * 0.5f, Projectile.scale, SpriteEffects.None, 0f);
			for (int i = 0; i < Projectile.oldRot.Length; i++)
			{
				...
				Main.spriteBatch.Draw(Request<Texture2D>("ProvidenceMod/Projectiles/Melee/StratusSpearGlow").Value, Projectile.Providence().oldCen[i] - Main.screenPosition, new Rectangle(0, 0, 146, 42), color, Projectile.oldRot[i], new Vector2(Projectile.width, Projectile.height) * 0.5f, Projectile.scale, SpriteEffects.None, 0f);
			}
			return false;
		}
