	...
	public class ProvidenceGlobalNPC : GlobalNPC
	{
		...
		public override void PostDraw(NPC npc, SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
		{
			...
			foreach (Texture2D buffT in buffs)
			{
				...
				{
					counter++;
					spriteBatch.Draw(buffT, drawPos + new Vector2(counter < 6 ? ((npc.width - 80f) / 2f) + offset + 8f : ((npc.width - 80f) / 2f) + (offset - 80f) + 8f, counter < 6 ? npc.height + 40f : npc.height + 56f), new Rectangle(0, 0, 32, 32), Color.White, 0, new Vector2(16, 16), 0.5f, SpriteEffects.None, 0f);
					offset += 16;
				}
			}
