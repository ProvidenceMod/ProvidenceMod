...
namespace Providence.PrimitiveTrails
{
	...
	public class Trail
	{
		...
		{
			void DrawTrail(TrailHelper manager);
		}
		public void Update()
		{
			TrailHelper.GetObjectValues(this, out (bool active, Vector2 position, Vector2 center, Vector2 velocity, float width, float height, float opacity) values);
		...
		public void Draw(Effect effect, GraphicsDevice device)
		{
			...
			for (int i = 1; i < points.Count; i++)
			{
				...
				float quotient2 = (int)(quotient * 10f) / 10f;
			...
			effect.Parameters["WorldViewProjection"].SetValue(ProvidenceMod.Trails.worldViewProjection);
		...
		public Vector2 CurveNormal(List<Vector2> points, int index)
		{
			...
			return Vector2.Normalize(points[index + 1] - points[index - 1]).TurnRight();
		}
	}
}
