	...
	public class ProvidenceSpriteSpawn : ModItem
	{
		...
		public override bool UseItem(Player player)
		{
			if (player.altFunctionUse == 2)
			{
				...
				y = 0;
				for (int i = 0; i < 20; i++)
					NewParticle(Main.MouseWorld, new Vector2(4f, 0f).RotatedBy(Main.rand.NextFloat(-360, 361).InRadians()), new GenericGlowParticle(), new Color(218, 70, 70, 0), 0.25f);
		...
		public override void PostDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
		{
			if (x != 0 && y != 0)
			{
				...
				Effect quantum = ProvidenceMod.Instance.GetEffect("Effects/DivinityShader");
				Effect outline = ProvidenceMod.Instance.GetEffect("Effects/Outline");
				Effect chroma = ProvidenceMod.Instance.GetEffect("Effects/ChromaAberration");

				Texture2D mbTex = GetTexture("ProvidenceMod/NPCs/PrimordialCaelus/PrimordialCaelus");
				Texture2D swirlTex = GetTexture("ProvidenceMod/ExtraTextures/Masks/SwirlTexture");
