...
namespace ProvidenceMod.Items.Weapons.Melee
{
  public class MoonCleaver : ModItem
  {
    ...
    public override void SetDefaults()
    {
      ...
      item.Providence().glowmaskTexture = ModContent.GetTexture("ProvidenceMod/Items/Weapons/Melee/MoonCleaverGlow");
      item.Providence().animationTexture = ModContent.GetTexture("ProvidenceMod/Items/Weapons/Melee/MoonCleaverAnimated");
    }
    public override void ModifyTooltips(List<TooltipLine> tooltips)
    {
      ...
      if (tt != null)
      {
        ...
        tt.text = split[0] + " fire " + split.Last();
      }
    }

    public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
    {
      const int numberProjectiles = 4; // 4 or 5 shots
      const float pos = -40f;
      for (int i = 0; i < numberProjectiles; i++)
      {
        ...
        float x = Main.mouseX + Main.screenPosition.X - vector2.X;
        // X position of the Mouse - the...true center? Of the Player

        float y = Main.mouseY + Main.screenPosition.Y - vector2.Y;
        // Y position of the Mouse

        float hyp = (float)Math.Sqrt((x * (double)x) + (y * (double)y));
        ...
        _ = Projectile.NewProjectile(vector2.X, vector2.Y, x2, y2, type, damage, knockBack, player.whoAmI, 0.0f, (float)Main.rand.Next(3));
    ...
    public override void PostDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, float rotation, float scale, int whoAmI)
    {
      ...
      spriteBatch.Draw(tex, new Vector2(item.position.X - Main.screenPosition.X + (item.width * 0.5f), item.position.Y - Main.screenPosition.Y + item.height - (tex.Height * 0.5f) - 2), tex.Frame(), Color.White, rotation, tex.Size() * 0.5f, scale, 0, 0); //draws our glowmask in the inventory. To see how to draw it in the world, see the ModifyDrawLayers method in ExamplePlayer.
    }
