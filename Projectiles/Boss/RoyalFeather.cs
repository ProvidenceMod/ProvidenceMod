...
namespace ProvidenceMod.Projectiles.Boss
{
  public class RoyalFeather : ModProjectile
  {
    ...
    public int spawnExtraProjectile;
    ...
    {
      if (projectile.Providence().shotBy == ModContent.ItemType<HarpyRod>())
      {
        projectile.hostile = false;
        projectile.friendly = true;
        ...
        projectile.Homing(npc, speedCap, default, default, turnStrength, 1500);
        ...
        for(spawnExtraProjectile = 0 ; spawnExtraProjectile < 1 ; spawnExtraProjectile++)
        {
          projectile.velocity = projectile.velocity.RotatedBy(10f.InRadians());
          _ = Projectile.NewProjectile(projectile.position, projectile.velocity.RotatedBy(-10f.InRadians()), projectile.type, projectile.damage, projectile.knockBack, projectile.owner);
        }
      }
      else
      {
